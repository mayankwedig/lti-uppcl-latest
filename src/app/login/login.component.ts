...
export class LoginComponent implements OnInit {
    ...
    if (this.forgotPassFrm.value.email != "") {
      ...
      if (this.forgotPassFrm.value.email1 != "") {
        ...
        if (errorExists == false) {
          ...
          this.toastr.error(
            this._translate.translate("Please fill required fields"),
            this._translate.translate("Failed!")
          );
        }
      } else {
        ...
        this.toastr.error(
          this._translate.translate("Please fill at least one required field."),
          this._translate.translate("Failed!")
        );
      }
    }
  ...
  forgotPassword(withSequityQues: boolean = false, data, apiUrl) {
    ...
    this.loder = true;
    this.loginService.forgotPassService(apiUrl, forgotPassData).subscribe(
      (response: any) => {
        this.loder = false;
        this.initForgotFormFrm();
        var res = response;
        if (res.authCode) {
          if (res.authCode == "200" && res.status == true) {
            ...
            if (OtpVerificationToken != null) {
              this.toastr.success(
                this._translate.translate(res.msg),
                this._translate.translate("Verification is successful!")
              );
            ...
            if (UserData.numberOfAccounts == 1) {
              ...
              this.Dashboard.getAccountDetails(accountId, (result: any) => {
                ...
                if (result.authCode == "200") {
                  this.loginService.getNotifyKeyWords().subscribe(
                    (res: any) => {
                      if (res.authCode == "200") {
                        //res.data_params
                        this.helper.setLocalStoragData(
                          "notifications",
                          JSON.stringify(res.data_params)
                        );
                      }
                    },
                    error => {}
  ...
  getSelectedQuestions(userName) {
    var fields = {};
    if (userName != "") {
      this.loginService.getUsersQuetion(userName).subscribe(
        (result: any) => {
          if (
            result.authCode == 200 &&
            result.status == true &&
